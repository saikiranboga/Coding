...
int main(){
	...
	fu(i,1,N-1){
		subs[i] = N-pos[i].idx-height[i];
		subs[i] += subs[i-1];
	}
	//rep(i,N)
	//	cout << subs[i] << "\t" << s.substr(pos[i].idx, N-pos[i].idx) << endl;

	int q,x, idd, len, prev;
	int *id;
	cin >> q;
	while(q--){
		cin >> x;
		id = lower_bound(subs,subs+N,x);
		idd = id-subs;
		if(idd!=0){
			len = height[idd];
			prev = subs[idd-1];
		...
		cout << s.substr(pos[idd].idx,len+x-prev) << endl;
	}
	return 0;
}