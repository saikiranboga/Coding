...
public:
    ...
    void refresh(int node_num, int st, int ed){
        if(tree[node_num]->lazy>0){
            tree[node_num]->lazy = 0;
            tree[node_num]->sum = (ed-st+1)-tree[node_num]->sum;
            if(st!=ed){
                tree[node_num*2]->lazy = !tree[node_num*2]->lazy;
                tree[node_num*2+1]->lazy = !tree[node_num*2+1]->lazy;
            }
        }
        return;
    }
    ...
    void update(int node_num, int st, int ed, int i, int j) {
        ...
        if(st>=i && ed<=j){
            tree[node_num]->lazy = !tree[node_num]->lazy;
            refresh(node_num, st, ed);
            return;
        }
...
int main(){
    ...
    while(Q--) {
        ...
        if(x>y)
            swap(x,y);
        if(type==0)
            ST->update(1, 1, N, x+1, y+1);
        else {
            int ans = ST->query(1, 1, N, x+1, y+1);
            printf("%d\n",ans);
        }
    }
}